...
const CourseBottomSheet: React.FC<CourseBottomSheetProps> = ({
  ...
  const defaultSelectedCourse =
    batches && batches.length > 0 ? batches[0].id : "";
  ...
  const courses = batches.map((batch) => ({
    ...
    duration:
      { 12: "1 year", 24: "2 years", 36: "3 years" }[batch.duration] ||
              ...
              <div className="space-y-4">
                {courses.length > 0 ? (
                  courses.map((course) => (
                    <div key={course.id}>
                      <div
                        className={`border rounded-lg p-4 flex items-center cursor-pointer justify-between transition-all ${
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="text-[#1d2939] text-sm md:text-xl font-bold">
                              {course.type}
                            </p>
                        ...
                        <div className="text-right">
                          ...
                          {course.discount > 0 && (
                            <p>
                              <span className="line-through text-[#1d2939] text-xs md:text-lg font-normal">
                                {course.originalPrice.toLocaleString()}
                              </span>
                              <span className="text-[#0e9a49] text-xs md:text-lg font-bold">
                                {" "}
                                ({course.discount}% OFF)
                              </span>
                            </p>
